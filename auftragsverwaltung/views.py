...
def invoice_finalize(request, pk):
    """
    Finalize an invoice or credit note (Echtdruck): assign number, set status
    to SENT, write the journal entry, and download PDF.

    This is an idempotent operation:
    - If document already has a number, it won't be changed
    - Status is set to SENT
    - Exactly one journal entry (Rechnungsausgangsjournal) is created
    - PDF is generated and returned for download

    Args:
        request: HTTP request
        pk: Primary key of the SalesDocument (invoice or credit note)
    ...
    # Validate this is a journal-relevant document (invoice or credit note)
    if get_document_kind(document) is None:
        return JsonResponse({
            'success': False,
            'error': f'Dokument ist weder Rechnung noch Gutschrift (Typ: {document.document_type.name})'
        }, status=400)

    try:
        # Finalize document (number, status, journal entry)
        document, was_modified = finalize_document(document)
        ...
        if was_modified:
            ActivityStreamService.add(
                ...
                title=f'{document.document_type.name} finalisiert: {document.number}',
            ...
            logger.info(f"Document {document.number} finalized by {request.user.username}")
        ...
        safe_type = ''.join(c if c.isalnum() or c in ('-', '_') else '_' for c in document.document_type.name)

        result = pdf_service.render(
            template_name=template_name,
            context=context,
            base_url=base_url,
            filename=f'{safe_type}_{safe_number}.pdf'
        )
        ...
        logger.info(f"Document {document.number} finalized and PDF generated by {request.user.username}")

        return response

    except ValueError as e:
        logger.warning(f"Document finalization failed: {str(e)}")
        return JsonResponse({
            'success': False,
            'error': str(e)
        }, status=400)
